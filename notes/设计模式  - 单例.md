...
该实现在多次序列化和序列化之后(反序化攻击)，不会得到多个实例,因为在反序列化时通过`Enum.valueOf(class,name)`获取现有的枚举对象。而其它实现需要使用 transient 修饰所有字段，并且实现序列化和反序列化的方法，反序化攻击还可以定义` readResolve` 方法来指定反序化出来的新对象。
