...
非对称密钥加密，又称公开密钥加密（Public-Key Encryption），加密和解密使用不同的密钥（不一定**只用**公钥加密私钥解密，要求是加密和解密是不同密钥即可，例如CA 证书用**私钥生成签名**，验证时用公钥解密然后在比较）。
...
1. 获取服务端的公钥（实际是证书包含公钥）
2. 客户端生成会话密钥 session key（对称加密密钥）并用服务器和公钥加密发送给服务端
3. 服务端 利用私钥解密得到 session key
4. 服务端客户端之间通过 session key 对称加密通信

<div align="center"> <img src="https://cs-notes-1256109796.cos.ap-guangzhou.myqcloud.com/How-HTTPS-Works.png" width="600"/> </div><br>

### 认证

在上一章节为保证对称加密的密钥安全送达，需要获取目标服务器的公钥来加密，但是这一过程可能被中间者篡改，所以需要对公钥进行认证。

实际上通过使用 **证书** （包含公钥以及一个签名）来对通信方进行认证。

数字证书认证机构（CA，Certificate Authority）是客户端与服务器双方都可信赖的第三方机构。

服务器的运营人员向 CA 提出公开密钥的申请，CA 在判明提出申请者的身份之后，会对已申请的**公开密钥**做**数字签名**（hash用自己**私钥**加密,私钥加密防止伪造签名），然后分配这个已签名的公开密钥，并将该公开密钥放入公开密钥证书后绑定在一起。

进行 HTTPS 通信时，服务器会把**证书**发送给客户端。客户端取得其中的公开密钥之后，先使用**数字签名**进行验证（用户ca的公钥解密比较hash值是否相等），如果验证通过，就可以开始通信了。
